...
static void function(FunctionType type) {
	...
	if(!check(TOKEN_RIGHT_PAREN)) {
		do {
			...
			uint8_t parameterConstant = parseVariable("Expect parameter name.");
			defineVariable(parameterConstant);
		} while (match(TOKEN_COMMA));
	}
	consume(TOKEN_RIGHT_PAREN, "Expect ')' after parameters.");

	// Function body
	consume(TOKEN_LEFT_BRACE, "Expect '{' before function start.");
	block();
...
static void ifStatement() {
	...
	expression();
	consume(TOKEN_RIGHT_PAREN, "Expect ')' ater condition.");
...
static void forStatement() {
	...
	if (!match(TOKEN_RIGHT_PAREN)) {
		...
		expression(); // why not expressionStatement()?
		emitByte(OP_POP);
